...
BASE   = r"C:\Users\zachs\OneDrive\Desktop\Wiki Dashboard Project"
OUT    = BASE + r"\WIN Abstract_Precision Oncology_Zach Schneider-Lynch.docx"
OUT_MD = BASE + r"\WIN_2026_abstract_precision_oncology.md"
...
def wc(sections):
    """Word and character count of an abstract body, section labels included."""
    t = " ".join(f"{lab} {txt}" for lab, txt in sections)
    t = re.sub(r"\s+", " ", t).strip()
    return len(t.split()), len(t)


def write_markdown():
    a_w, a_c = wc(VERSION_A)
    b_w, b_c = wc(VERSION_B)
    L = []
    L.append("# WIN Symposium 2026 \u2014 Poster Abstract (revised for precision oncology)\n")
    L.append("Deadline: **17 September 2026**. Portal: mcigroup-winsymposium2026.eventsairsite.com\n")
    L.append("The portal does not publish a word or character limit publicly, so both versions below")
    L.append("are verified by count. **Version A is close to the length of the abstract already")
    L.append("submitted (330 words), so it fits whatever limit that was written to.** Version B is the")
    L.append("fuller treatment, if the portal allows roughly 500.\n")
    L.append("> Generated by `tools_make_abstract_docx.py`, which also produces the .docx.")
    L.append("> Edit that script rather than this file, so the two cannot drift apart.\n")
    L.append("---\n")
    L.append("## Title\n")
    L.append(f"**Short ({len(TITLE_SHORT)} characters):**\n\n{TITLE_SHORT}\n")
    L.append(f"**Full ({len(TITLE_FULL)} characters):**\n\n{TITLE_FULL}\n")
    L.append("---\n")
    L.append(f"## Version A \u2014 {a_w} words, {a_c:,} characters (recommended)\n")
    for lab, txt in VERSION_A:
        L.append(f"**{lab}** {txt}\n")
    L.append("---\n")
    L.append(f"## Version B \u2014 {b_w} words, {b_c:,} characters\n")
    for lab, txt in VERSION_B:
        L.append(f"**{lab}** {txt}\n")
    L.append("---\n")
    L.append("## Changes from the previous version, and why\n")
    for lead, rest in NOTES:
        L.append(f"- **{lead.strip()}** {rest}")
    L.append("")
    L.append("## Figures held in reserve (defensible, not in the abstract)\n")
    for item in RESERVE:
        L.append(f"- {item}")
    L.append("")
    L.append("## Known limitations, if asked\n")
    for item in LIMITS:
        L.append(f"- {item}")
    L.append("")
    L.append("## References supporting the Background\n")
    L.append("Each Background claim was checked against the literature. Items marked VERIFIED were")
    L.append("confirmed against full text; the rest were identified via search and should be")
    L.append("confirmed before submission.\n")
    for i, (cite, note) in enumerate(REFS, 1):
        L.append(f"{i}. {cite}")
        L.append(f"   - *{note}*\n")
    L.append("## Considered and deliberately not used\n")
    for cite, note in NOT_USED:
        L.append(f"- {cite}")
        L.append(f"  - *{note}*\n")
    io.open(OUT_MD, "w", encoding="utf-8").write("\n".join(L))
    print("Saved:", OUT_MD)
    print(f"  Version A: {a_w} words / {a_c:,} chars")
    print(f"  Version B: {b_w} words / {b_c:,} chars")
